<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="数据集" min-width="150px">
        <template slot-scope="{ row }">
      ...
      <el-table-column label="数据挖掘实验" min-width="150px">
        <template slot-scope="{ row }">
      ...
      <el-table-column label="任务" min-width="150px">
        <template slot-scope="{ row }">
      ...
      <el-table-column label="导出模型" min-width="150px">
        <template slot-scope="{ row }">
    ...
    <el-dialog title="新增项目" :visible.sync="dialogFormVisible" width="30%">
      <el-form
        ref="dataForm"
        :rules="rules"
        :model="temp"
...
<script>
import { getXMGLList, addXMGL, deleteXMGL, etchPv, createArticle, updateArticle } from '@/api/xmgl'
...
export default {
  ...
  methods: {
    getList() {
      ...
      getXMGLList(this.listQuery).then(response => {
    ...
    handleDatasets(row) {
      // return
      this.$router.push({ path: '/gzkj/datasets', query: { id: row.id }})
    },
    handleWorkfiles(row) {
      // return
      this.$router.push({ path: '/gzkj/workfiles', query: { id: row.id }})
    },
    handleJobs(row) {
      return
      this.$router.push({ path: 'jobs', query: { id: row.id }})
    },
    handleModels(row) {
      return
      this.$router.push({ path: 'models', query: { id: row.id }})
    },
    // handleWorkfiles(row) {
    //   this.$router.push({path: 'datasets', query: {id: row.id}})
